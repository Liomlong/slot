...
export async function GET(request: Request) {
  ...
  try {
    ...
    const res = await pool.query('SELECT points, usdt, spins_left, last_spin_time, username FROM users WHERE tg_id = $1', [tgId]);
    ...
    if (res.rows.length === 0) {
      ...
      const updateUser = await pool.query(
        'UPDATE users SET username = $1 WHERE tg_id = $2 RETURNING points, usdt, spins_left, last_spin_time, username',
        [username, tgId]
      );
